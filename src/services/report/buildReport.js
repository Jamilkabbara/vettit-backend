...
/**
 * The disclaimer asserted "generated by AI synthetic respondents calibrated to
 * the audience spec" unconditionally. On a mission where no responses were
 * recorded that is a FALSE STATEMENT printed on the report and every export -
 * it describes work that did not happen. Branch on whether a sample exists.
 */
const NO_SAMPLE_DISCLAIMER =
  'No respondent data was recorded for this mission, so nothing here is supported by a sample. Any figure shown is carried over from the mission setup, not measured. Treat this report as incomplete and contact support before relying on it.';

function disclaimerFor(sampleN) {
  return (Number(sampleN) > 0) ? DISCLAIMER : NO_SAMPLE_DISCLAIMER;
}
  ...
  return sanitizeDashesDeep({
    ...
    header: {
      ...
      sample: {
        ...
        // `distinctPersonas && ...` made ZERO fall to 'indicative', the
        // STRONGER posture - same fail-open shape as statGate's `N > 0`.
        // No sample is the weakest posture there is.
    ...
    methodology_disclaimer: disclaimerFor(distinctPersonas || (analysis && analysis.n) || 0),
  });
}
