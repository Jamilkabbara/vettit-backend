 ...
 */
async function loadPersistedResponseKeys(supabase, missionId) {
  const keys = new Set();
 ...
 */
async function persistResponseRows(supabase, missionId, rows, opts = {}) {
  const attempted = Array.isArray(rows) ? rows.length : 0;
  if (attempted === 0) return { attempted: 0, skipped: 0, sent: 0, error: null };

  const caller = opts.caller || 'persistResponseRows';
  let keys = opts.knownKeys instanceof Set ? opts.knownKeys : null;
  if (!keys) {
    const loaded = await loadPersistedResponseKeys(supabase, missionId);
    if (loaded.error) {
      ...
      logger.error('persistResponseRows: could not read existing keys; refusing to insert blind', {
  ...
  if (skipped > 0) {
    logger.warn('persistResponseRows: skipped rows already persisted for this mission', {
  ...
  for (let i = 0; i < fresh.length; i += INSERT_CHUNK) {
    ...
    let { error } = await supabase
      .from('mission_responses')
      .upsert(chunk, { onConflict: RESPONSE_CONFLICT_TARGET, ignoreDuplicates: true });
    if (error && isMissingConflictTarget(error)) {
      ...
      logger.warn('persistResponseRows: unique index absent — falling back to plain insert', {
        missionId, caller, err: error.message,
      });
      ({ error } = await supabase.from('mission_responses').insert(chunk));
    }
    if (error && !firstError) firstError = error;
    if (error) {
      logger.error('persistResponseRows: chunk insert failed', {
