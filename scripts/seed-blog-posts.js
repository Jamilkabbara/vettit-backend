// One-off seeder for Pass 35 A7 — runs via the backend's existing
// Supabase service-role client.
const path = require('path');
require('dotenv').config({ path: path.join('/Users/jamilkabbara/vettit-backend', '.env.local') });
require('dotenv').config({ path: path.join('/Users/jamilkabbara/vettit-backend', '.env') });

const supabase = require('/Users/jamilkabbara/vettit-backend/src/db/supabase');

const POSTS = [
  {
    slug: 'product-market-fit-synthetic-respondents-2026',
    title: 'How to test product-market fit in 2026 with synthetic respondents',
    excerpt: 'Concept tests used to take weeks and thousands of dollars. Synthetic respondents collapse the loop to minutes for $9-99 — what changes, what stays the same.',
    tag: 'Methodology',
    emoji: '🧪',
    published: true,
    auto_generated: false,
    body_markdown: `## Why concept tests still matter
...
Drop a concept idea into [/setup](/setup) and you'll have a 5-question survey + 5-respondent test in your inbox in about 10 minutes.`,
  ...
  {
    slug: 'van-westendorp-pricing-saas-2026',
    title: 'Van Westendorp pricing for SaaS: what $9-35 missions look like in practice',
    excerpt: 'The 4-question PSM battery + 5-anchor Gabor-Granger ladder, run on synthetic respondents. What you can actually learn at the entry tier — and what you can\'t.',
    tag: 'Methodology',
    emoji: '💰',
    published: true,
    auto_generated: false,
    body_markdown: `## The instrument
...
[Start a pricing mission](/setup?goal=pricing). Bring a 1-paragraph product description and a rough price band; the AI fills in the rest.`,
  ...
  {
    slug: 'ad-creative-attention-frame-by-frame-2026',
    title: 'Ad creative attention: replacing 4-week panel research with frame-by-frame AI',
    excerpt: 'Attention prediction at 24 emotion dimensions, distinctive brand asset scoring, channel fit benchmarks. What synthetic-respondent creative testing can and cannot do.',
    tag: 'Methodology',
    emoji: '🎬',
    published: true,
    auto_generated: false,
    body_markdown: `## The traditional creative test
...
If you're shipping ads tomorrow and you didn't do any pre-test, run a $19-39 Creative Attention pass first. It will catch the worst mistakes — wrong stopping moment, brand reads at frame 4 instead of frame 1, emotional arc inverted. Different jobs, different tools.
...
(async () => {
  for (let i = 0; i < POSTS.length; i++) {
    const post = POSTS[i];
    const publishedAt = new Date(Date.now() - i * 86400 * 1000).toISOString();
